...
double Timer::GetAverageTime()
{
  return _total_time / (double)_counter;
}
...
void Timer::PrintAverageTiming()
{
  if (_counter > 0)
  {
    std::cout << "Timer " << _name << " Average Time [ms]: " << 1000 * this->GetAverageTime() << " over " << _counter
              << " samples" << std::endl;
  }
}
