...
void O3dColorIcp::LoadSettingsFromROSNodeHandle(ros::NodeHandle nh)
{
  ...
  assertm(max_iter_.size() == (voxel_radius_.size() == normal_radius_.size()), "Number of entries provided for maxt "
                                     ...
                                     const Eigen::Matrix4f& guess)
{
  ...
  if (received_first_point_cloud_)
  {
    ...
    for (int i = 0; i < (int)voxel_radius_.size(); i++)
    {
      ...
      auto result_icp = RegistrationColoredICP(*((*curr_downsampled_pcs_ptr)[i]), *((*prev_downsampled_pcs_ptr_)[i]),
      ...
      std::cout << "[Colored ICP Iteration " << i + 1 << " Results] Inlier RSME: " << result_icp.inlier_rmse_
...
O3dColorIcp::GetDownsampledPcs(const open3d::geometry::PointCloud& input_cloud, const std::vector<double>& voxel_radius,
                               const std::vector<double>& normal_radius)
{
  ...
  assertm(voxel_radius.size() == normal_radius.size(), "Number of entries provided for maxt "
                                                       "iter and voxel voxel_radius "
                                                       "must "
                                                       "be the same");
  ...
  for (int i = 0; i < voxel_radius.size(); i++)
  {
    ...
    std::shared_ptr<open3d::geometry::PointCloud> current_pc_ptr = std::make_shared<open3d::geometry::PointCloud>();
    ...
    current_pc_ptr->EstimateNormals(open3d::geometry::KDTreeSearchParamHybrid(n_rad * 2, (int)settings_["max_"
                                                                                                        "neare"
                                                                                                        "s"
                                                                                                        "t_"
                                                                                                        "neigh"
                                        ...
                                        const Eigen::Matrix4d& Transformation)
{
  ...
  *target_ptr = target;
  ...
  open3d::visualization::DrawGeometries({ source_transformed_ptr, target_ptr }, "Registration result", 640, 480, 50, 50,
                                        false);
}
