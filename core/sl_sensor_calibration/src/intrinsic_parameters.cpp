...
IntrinsicParameters::IntrinsicParameters(const cv::Matx33f& intrinsic_mat,
                                         const cv::Vec<float, 5>& lens_distortion,
                                         double calibration_error, int resolution_x,
                                         int resolution_y)
...
IntrinsicParameters::IntrinsicParameters(const std::string& filename) { Load(filename); }
...
bool IntrinsicParameters::Save(const std::string& filename) {
  ...
  if (file_extension == "xml") {
    ...
    if (fs.isOpened()) {
      fs << "intrinsic_mat" << cv::Mat(intrinsic_mat_) << "lens_distortion"
         << cv::Mat(lens_distortion_) << "calibration_error" << calibration_error_ << "resolution_x"
         << resolution_x_ << "resolution_y" << resolution_y_;
      fs.release();
      return true;
    }
  } else {
    std::cerr << "[IntrinsicParameters] Error: File extension not .xml, got " << file_extension
              << " instead." << std::endl;
  }
...
const cv::Matx33f& IntrinsicParameters::intrinsic_mat() const { return intrinsic_mat_; }

const cv::Vec<float, 5>& IntrinsicParameters::lens_distortion() const { return lens_distortion_; }

const double& IntrinsicParameters::calibration_error() const { return calibration_error_; }

const int& IntrinsicParameters::resolution_x() const { return resolution_x_; }

const int& IntrinsicParameters::resolution_y() const { return resolution_y_; }
