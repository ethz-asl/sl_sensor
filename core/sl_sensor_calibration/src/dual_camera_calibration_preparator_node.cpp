...
int main(int argc, char** argv)
{
  ...
  private_nh.param<std::string>("sec_projector_folder_name", sec_projector_folder_name, sec_projector_folder_name);
  private_nh.param<std::string>("directories", directories, directories);

  private_nh.param<std::string>("bundle_adjustment_problem_output_filename", bundle_adjustment_problem_output_filename,
  ...
  for (const auto& directory : directories_vec)
  {
    ...
    if (!DirectoryExists(pri_cam_shading_directory.c_str()))
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Shading Directory " +
                                   pri_cam_shading_directory + " does not exist, skipping this folder";
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Mask Directory " + pri_cam_mask_directory +
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Shading Directory " +
                                   sec_cam_shading_directory + " does not exist, skipping this folder";
      ROS_INFO("%s", status_message.c_str());
      continue;
    }
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Mask Directory " + sec_cam_mask_directory +
                                   " does not exist, skipping this folder";
      ROS_INFO("%s", status_message.c_str());
      continue;
    }
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Up Directory " + pri_projector_up_directory +
                                   " does not exist, skipping this folder";
      ROS_INFO("%s", status_message.c_str());
      continue;
    }
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Vp Directory " + pri_projector_vp_directory +
                                   " does not exist, skipping this folder";
      ROS_INFO("%s", status_message.c_str());
      continue;
    }
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Up Directory " + sec_projector_up_directory +
                                   " does not exist, skipping this folder";
      ROS_INFO("%s", status_message.c_str());
      continue;
    }
    ...
    {
      std::string status_message = "[DualCameraCalibrationPreparatorNode] Vp Directory " + sec_projector_vp_directory +
                                   " does not exist, skipping this folder";
      ROS_INFO("%s", status_message.c_str());
      continue;
    }

    for (const auto& entry : std::experimental::filesystem::directory_iterator(pri_cam_shading_directory))
    {
      ...
      bool pri_up_success = GetCvMatFromXml(pri_projector_up_directory + "/" + stem + ".xml", "up", pri_up);
      bool pri_vp_success = GetCvMatFromXml(pri_projector_vp_directory + "/" + stem + ".xml", "vp", pri_vp);
      bool sec_up_success = GetCvMatFromXml(sec_projector_up_directory + "/" + stem + ".xml", "up", sec_up);
      bool sec_vp_success = GetCvMatFromXml(sec_projector_vp_directory + "/" + stem + ".xml", "vp", sec_vp);

      // If all read successfully, add to calibrator
      if (pri_shading_success && sec_shading_success && pri_mask_success && sec_mask_success && pri_up_success &&
          pri_vp_success && sec_up_success && sec_vp_success)
      {
        std::string label = directory + "|" + stem;
        bool load_success = preparator.AddSingleCalibrationSequence(pri_shading, pri_mask, pri_up, pri_vp, sec_shading,
                                                                    sec_mask, sec_up, sec_vp, label);
