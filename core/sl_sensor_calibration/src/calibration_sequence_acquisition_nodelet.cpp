...
void CalibrationSequenceAcquisitionNodelet::onInit() {
  ...
  nh_.param<std::string>("erase_sequence_service_name", erase_sequence_service_name_,
                         erase_sequence_service_name_);
  nh_.param<std::string>("grab_sequence_service_name", grab_sequence_service_name_,
                         grab_sequence_service_name_);

  private_nh_.param<std::string>("image_array_sub_topic", image_array_sub_topic_,
                                 image_array_sub_topic_);
  ...
  // Setup subscribers
  image_array_sub_ = nh_.subscribe(image_array_sub_topic_, 10,
                                   &CalibrationSequenceAcquisitionNodelet::ImageArrayCb, this);
  ...
  for (int i = 0; i < number_cameras_; i++) {
    ...
    std::string shading_topic_name =
        "calibration_data_acquisition_nodelet/cam" + number + "/shading";
  ...
  image_synchroniser_client_ =
      nh_.serviceClient<sl_sensor_image_acquisition::CommandImageSynchroniser>(
          image_synchroniser_service_name_);
  projector_client_ =
  ...
  erase_sequence_client_ = nh_.advertiseService(
      erase_sequence_service_name_,
      &CalibrationSequenceAcquisitionNodelet::ProcessEraseSequenceCommand, this);
  grab_next_sequence_client_ = nh_.advertiseService(
      grab_sequence_service_name_,
      &CalibrationSequenceAcquisitionNodelet::ProcessGrabNextSequenceCommand, this);

  // Setup folders
  GenerateDataFolders();

  // Start thread to handle first command to image synchroniser
  initialisation_thread_ptr_ = std::shared_ptr<std::thread>(
      new std::thread(std::bind(&CalibrationSequenceAcquisitionNodelet::Init, this)));
}
...
void CalibrationSequenceAcquisitionNodelet::ImageArrayCb(
  ...
  if ((int)(image_arr_ptr->data.size()) != (int)(number_cameras_ * 4)) {
    ROS_INFO(
        "[CalibrationSequenceAcquisitionNodelet] Number of images in image array does not "
        "correspond to ("
  ...
  for (int i = 0; i < number_cameras_; i++) {
    ...
    bool success =
        cv::findChessboardCorners(image_vec_buffer_[i * 4 + 3], checkerboard_size_,
                                  checkerboard_coordinates, cv::CALIB_CB_ADAPTIVE_THRESH);

    if (!success) {
      std::string info_string =
          "[CalibrationSequenceAcquisitionNodelet] Failed to extract checkerboard from camera " +
          std::to_string(i + 1);
  ...
  for (int i = 0; i < number_cameras_; i++) {
    ...
    cv_bridge::CvImage(std_msgs::Header(), "8UC1", image_vec_buffer_[i * 4 + 2])
        .toImageMsg(*mask_display_ptr);
    image_pubs_[i][1].publish(mask_display_ptr);

    // Publish decoded horizontal projector (up) coordinates (first convert float matrix to an 8 bit
    ...
    // Publish decoded vertical projector (vp) coordinates (first convert float matrix to an 8 bit
    // image for visualisation)
  ...
  std::string message =
      "[CalibrationSequenceAcquisitionNodelet] Checkerboard detection for this round: ";
  for (int i = 1; i <= number_cameras_; i++) {
    message += ("Cam" + std::to_string(i) + ": " +
                std::to_string(extracted_checkerboard_success_vec[i - 1]) + " ");
  }
...
bool CalibrationSequenceAcquisitionNodelet::GenerateDataFolders() {
  ...
  if (success) {
    for (int i = 1; i <= number_cameras_; i++) {
      ...
      success = success && std::experimental::filesystem::create_directory(
                               save_directory_ + save_filename_ + "/" + "cam" + number);
      success =
          success && std::experimental::filesystem::create_directory(
                         save_directory_ + save_filename_ + "/" + "cam" + number + "/" + "mask");
      success =
          success && std::experimental::filesystem::create_directory(
                         save_directory_ + save_filename_ + "/" + "cam" + number + "/" + "shading");
      success = success && std::experimental::filesystem::create_directory(
                               save_directory_ + save_filename_ + "/" + "proj" + number);
      success = success &&
                std::experimental::filesystem::create_directory(save_directory_ + save_filename_ +
                                                                "/" + "proj" + number + "/" + "up");
      success = success &&
                std::experimental::filesystem::create_directory(save_directory_ + save_filename_ +
                                                                "/" + "proj" + number + "/" + "vp");
...
void CalibrationSequenceAcquisitionNodelet::ProcessFloatImage(const cv::Mat& input_image,
                                                              cv::Mat& output_image) {
...
void CalibrationSequenceAcquisitionNodelet::SendCommandForNextCalibrationSequence() {
  ...
  srv.request.delay_ms = 500;  // At least half a second of delay to take into account that the
                               // projector is in a transient state between for some time between
                               // projecting nothing to an illuminated pattern
...
void CalibrationSequenceAcquisitionNodelet::SendProjectorCommand(const std::string& command,
                                                                 int pattern_no) {
...

{
  ...
  if (image_vec_buffer_.empty()) {
    ROS_INFO(
        "[CalibrationSequenceAcquisitionNodelet] There is no calibration sequence to be erased!");
  } else {
