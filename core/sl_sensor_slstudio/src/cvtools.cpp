...
namespace slstudio {
namespace cvtools {
void phase_correlate_register_image_sequence(const std::vector<cv::Mat> &image_sequence,
...
void convert_mser_rects_to_keypoints(const std::vector<cv::Rect> &rect_vec,
...
void get_descriptor_matches(cv::Ptr<cv::DescriptorMatcher> matcher_ptr,
...
void keep_only_symmetric_matches(const std::vector<cv::DMatch> &matches12,
...
void cv_dmatches_to_pcl_correspondences(const std::vector<cv::DMatch> &dmatches,
...
void pcl_correspondences_to_cv_dmatches(const pcl::Correspondences &correspondences,
...
void filter_correspondences_by_median_vertical_pixel_shift(
...
void update_correspondences_by_deviation_from_median(
...
void remove_nan_correspondences(const pcl::PointCloud<pcl::PointXYZ> &static_pc,
  ...
  for (const auto &correspondence : correspondences) {
    ...
    bool not_nan_entry = !(isnan(source_pt.x) || isnan(source_pt.y) || isnan(source_pt.z) ||
