...
void LinearMotionCompensationNodelet::onInit() {
  ...
  if (pattern_direction_ == "horizontal") {
    ...
    ROS_WARN(
        "[LinearMotionCompensationNodelet] Invalid/No pattern direction provided! Defaulting "
...
void LinearMotionCompensationNodelet::ImageArrayCb(
  ...
  if (filter_id_.empty() || (!filter_id_.empty() && filter_id_ == image_arr_ptr->id)) {
    ...
    // should be larger than reference index (so index is not out of range). If these conditions are
    // not met, we do not proceed with processing
    bool equal_number_images_per_camera =
        (image_arr_ptr->data.size() % (size_t)image_arr_ptr->number_cameras) == 0;
    ...
    if ((size_t)reference_index_ >= images_per_camera) {
      ...
      ROS_WARN(
          "[LinearMotionCompensationNodelet] Provided reference indice is larger than the number "
          "of images for "
    ...
    if (!equal_number_images_per_camera) {
      proceed_with_alignment = false;
      ROS_WARN(
          "[LinearMotionCompensationNodelet] Unequal number of images from each camera, this is "
          "not expected, "
          "ignoring this image array message");
    }

    // Note 1: For the current implmentation, this nodelet only processes one set of camera images
    // and outputs it ( images from other cameras will be ignored)
    if (proceed_with_alignment) {
      ...
      // Add to unaligned_images the camera images that we want to align and also keep track of the
      // encoding
      std::vector<cv::Mat> unaligned_images;
      std::vector<std::string> encoding_vec;
      for (size_t i = camera_index_ * images_per_camera;
           i < camera_index_ * images_per_camera + images_per_camera; i++) {
      ...
      PhaseCorrelateAlignImageSequence(unaligned_images, aligned_images, reference_index_,
                                       subsample_factor_, shifting_option_);

      // Publish aligned images

      // Note 2: We tag the output images with the timestamp of the reference image. This is
      // important in matching the acquisition time of the point cloud to its position it the
      // trajectory Note 3: As explained in Note 1, we are only dealing with image sequence from one
      // camera only, hence the number_cameras argument is set to 1
      PublishCvMatVec(output_pub_, aligned_images, image_arr_ptr->header.frame_id,
                      image_arr_ptr->data[reference_index_].header.stamp, ros::Time::now(),
                      encoding_vec, 1, image_arr_ptr->id);
    }
  }
}
