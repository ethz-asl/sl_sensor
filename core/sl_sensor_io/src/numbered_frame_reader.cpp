...
namespace sl_sensor {
namespace io {
NumberedFrameReader::NumberedFrameReader(const std::string &folder_directory,
                                         const std::string &file_title,
                                         const std::string &file_format, int frame_no_start,
                                         int frame_no_end, int frame_increment, int pat_no_min,
                                         int pat_no_max)
...
bool NumberedFrameReader::GetNextImageSequence(std::vector<cv::Mat> &image_sequence) {
  int frame_to_read =
      (last_read_frame_no_ < 0) ? frame_no_start_ : last_read_frame_no_ + frame_increment_;

  if (frame_to_read > frame_no_end_) {
    return false;
  };

  ReadImageSequence(folder_directory_, file_title_, file_format_, frame_to_read, pat_no_min_,
                    pat_no_max_, image_sequence);
...
void NumberedFrameReader::reset() { last_read_frame_no_ = -1; };

void NumberedFrameReader::ReadImageSequence(const std::string &folder_directory,
                                            const std::string &file_title,
                                            const std::string &file_format, int frame_no,
...

{
  ...
  for (int i = pat_no_min; i <= pat_no_max; i++) {
    ...
    std::string full_filename =
        folder_directory + file_title + "_" + frame_no_str + "_" + pat_no_str + file_format;
