...
void TriangulatorNodelet::onInit() {
  ...
  private_nh_.param<std::string>("triangulation_camera_parameters_filename",
  ...
  private_nh_.param<std::string>("colour_camera_parameters_filename",
                                 colour_camera_parameters_filename_,
                                 colour_camera_parameters_filename_);

  private_nh_.param<std::string>("frame_camera_parameters_filename",
  ...
  // Setup Triangulator
  if (colour_shading_enabled_) {
    triangulator_ptr_ = std::make_unique<Triangulator>(
        projector_parameters, triangulation_camera_parameters, colour_camera_parameters);
  } else {
    triangulator_ptr_ =
        std::make_unique<Triangulator>(projector_parameters, triangulation_camera_parameters);
  }

  CameraParameters
      frame_camera_parameters;  // This contains transformation information between the projector
                                // and the camera in which you want to set as the sensor frame
  if (!frame_camera_parameters_filename_.empty()) {
    if (frame_camera_parameters.Load(frame_camera_parameters_filename_)) {
      ...
      cv::Mat cv_mat_transform_frame_tri =
          frame_camera_parameters.GetInverseTransformationMatrix() *
  ...
  image_array_sub_ =
      nh_.subscribe(image_array_sub_topic_, 10, &TriangulatorNodelet::ImageArrayCb, this);
};

void TriangulatorNodelet::ImageArrayCb(
    const sl_sensor_image_acquisition::ImageArrayConstPtr& image_array_ptr) {
  // If camera-projector setup, we expect 4 images. If there is a secondary camera providing colour
  // shading, we expect 5 images
  size_t expected_images = (colour_shading_enabled_) ? 5 : 4;
  if (expected_images != image_array_ptr->data.size()) {
    ROS_INFO(
        "[TriangulatorNodelet] Number of images in image array does not match requirements for "
        "triangulator. "
  ...
  if (colour_shading_enabled_) {
    // Triangulate (coloured shading image should be contained as the last image in the image array
    // message)
    auto pc_ptr = triangulator_ptr_->TriangulateColour(
        cv_img_ptr_vec.at(0)->image, cv_img_ptr_vec.at(1)->image, cv_img_ptr_vec.at(2)->image,
        cv_img_ptr_vec.at(4)->image);
    ...
    auto pc_ptr = triangulator_ptr_->TriangulateMonochrome(
        cv_img_ptr_vec.at(0)->image, cv_img_ptr_vec.at(1)->image, cv_img_ptr_vec.at(2)->image,
        cv_img_ptr_vec.at(3)->image);
