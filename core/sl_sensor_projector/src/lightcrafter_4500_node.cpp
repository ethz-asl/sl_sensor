 ...
 * Usage:
 * command - "white", "black" or a pattern name. White and black will display a bright and dark
 * projection respecitvely. pattern_no - Indice of the projection in the pattern sequenc to be
 * displayed.
 *
 * @param req
 * @param res
...
int main(int argc, char **argv) {
  ...
  nh.param<std::string>("projector_yaml_directory", projector_yaml_directory,
  ...
  if (!init_success) {
    ...
    ROS_WARN(
        "[Lightcrafter4500Node] No projector service name provided in YAML file! Terminating "
        "node.");
  } else {
    ros::ServiceServer service = nh.advertiseService(service_name, CommandProjector);

    ROS_INFO(
        "[Lightcrafter4500Node] Projector initialisation succeeded! You can start sending "
        "commands");
