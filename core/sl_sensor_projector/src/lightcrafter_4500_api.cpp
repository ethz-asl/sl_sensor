...
Lightcrafter4500Api::~Lightcrafter4500Api() { this->Close(); }
...
int Lightcrafter4500Api::SendPatternSequence(unsigned int exposure_period_us,
  ...
  for (i = 0; i < (int)pattern_store_.size(); i++) {
    ...
    **/
    ...
    if (DLPC350_AddToPatLut(curr_pattern.trigger_type, curr_pattern.pattern_number,
  ...
  if (DLPC350_SetPatternConfig(num_lut_entries, repeat_mode, num_patterns_trigout2,
...
int Lightcrafter4500Api::SetPatternSequenceStart() { return SetPatternSequenceMode(2); }

int Lightcrafter4500Api::SetPatternSequencePause() { return SetPatternSequenceMode(1); }

int Lightcrafter4500Api::SetPatternSequenceStop() { return SetPatternSequenceMode(0); }
...
void Lightcrafter4500Api::CheckAndFixBufferSwaps() {
  ...
  for (int i = 0; i < (int)pattern_store_.size(); i++) {
    ...
    if (prev_img_indice != curr_img_indice && !pattern_store_[i].buffer_swap) {
      ...
      std::cout << "Pattern with indice " << i << ": Change Buffer Swap to true since image indice "
                << curr_img_indice << " is different from the previous indice " << prev_img_indice
                << std::endl;
    }
...
int Lightcrafter4500Api::SetPatternSequence(
    const std::vector<LightcrafterSinglePattern>& pattern_vec) {
  pattern_store_.clear();
  pattern_store_ = pattern_vec;
  return 0;
}

int Lightcrafter4500Api::PlayPatternSequence(
    const std::vector<LightcrafterSinglePattern>& pattern_vec, unsigned int exposure_period_us,
    unsigned int frame_period_us) {
  // Set pattern
