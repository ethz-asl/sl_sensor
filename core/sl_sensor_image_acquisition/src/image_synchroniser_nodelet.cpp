...
void ImageSynchroniserNodelet::onInit() {
  // Get ROS nodes
  nh_ = getMTNodeHandle();  // Need to use multi threaded node so callbacks still work while nodelet
                            // is grouping images
  ...
  nh_.param<std::string>("projector_service_name", projector_service_name_,
                         projector_service_name_);
  ...
  private_nh_.param<std::string>("projector_topic", projector_timing_sub_topic_,
                                 projector_timing_sub_topic_);
  ...
  private_nh_.param<std::string>("image_topics", image_topics_full_string,
                                 image_topics_full_string);
  auto image_topics = SplitString(image_topics_full_string, std::string(" "));

  // Set up subscribers
  image_array_pub_ =
      nh_.advertise<sl_sensor_image_acquisition::ImageArray>(image_array_pub_topic_, 10);
  projector_timing_sub_ = nh_.subscribe(projector_timing_sub_topic_, 10,
                                        &ImageSynchroniserNodelet::ProjectorTimeCb, this);
  synchroniser_service_ =
      nh_.advertiseService(image_synchroniser_service_name_,
  ...
  for (const auto& topic : image_topics) {
    ...
    auto temp_grouper_ptr = std::make_unique<ImageGrouper>(topic, 1, image_trigger_period_,
                                                           lower_bound_tol_, upper_bound_tol_);
  ...
  // Init Service Client to projector, the projector service name is retrieved from the Projector
  // YAML file
  if (projector_service_name_.empty()) {
    ROS_INFO(
        "[ImageSynchroniserNodelet] Empty projector service name, service may not be set up "
        "correctly!");
  }
  projector_client_ =
      nh_.serviceClient<sl_sensor_projector::CommandProjector>(projector_service_name_);
  SendProjectorCommand("black", 0);

  // Create thread for main loop
  main_loop_thread_ptr_ = std::shared_ptr<std::thread>(
      new std::thread(std::bind(&ImageSynchroniserNodelet::MainLoop, this)));
}
...
void ImageSynchroniserNodelet::ProjectorTimeCb(
  ...
  if (command == "start") {
    ...
    if (!fixed_pattern_name_.empty() && fixed_pattern_name_ != pattern_name) {
      ...
      if (pattern_name.empty()) {
        message =
            "[ImageSynchroniserNodelet] Service call contains an empty pattern name entry. Will "
            "project the "
            "fixed pattern " +
            fixed_pattern_name_ + " instead.";
      } else {
        message = "[ImageSynchroniserNodelet] Service call contains a pattern name " +
                  pattern_name + " that is different from the fixed pattern name provided " +
                  fixed_pattern_name_ + ". Will project " + fixed_pattern_name_ + " instead.";
      }
    ...
    bool pattern_exists =
        projector::Lightcrafter4500::PatternExists(projector_config_, pattern_name);
    ...
    if (pattern_exists) {
      ...
      // If hardware triggered, we configure projector and project the entire image sequence and
      // sleep for a bit so projector has time to respond
      ...
      for (const auto& grouper_ptr : image_grouper_ptrs_) {
        grouper_ptr->SetImagesPerGroup((synchroniser_state_.is_hardware_trigger)
                                           ? synchroniser_state_.number_images_per_scan
                                           : 1);
        grouper_ptr->Start();
      }
    ...
    ROS_INFO(
        "[ImageSynchroniserNodelet] Received a service call with an empty command, did you forget "
        "to fill it in?");
...
bool ImageSynchroniserNodelet::ExecuteCommandHardwareTrigger() {
  ...
  for (auto projector_time_it = std::rbegin(projector_time_buffer_);
    ...
    for (const auto& grouper_ptr : image_grouper_ptrs_) {
      ...
      // Note, last argument is false because we do not want to clear the image buffer of the
      // ImageGrouper prematurely. We need to make sure the other cameras are successful before
      // clearing
    ...
    // If all image groupers are successful for this projector time, we break the loop and start
  ...
  if (success) {
    ...
    for (size_t i = 0; i < image_grouper_ptrs_.size(); i++) {
      image_grouper_ptrs_[i]->ClearAllImagesFromBufferBeforeTiming(
          nested_img_ptr_vec[i].back()->header.stamp);
    }
...
void ImageSynchroniserNodelet::ClearAllProjectorTimingsFromBufferBeforeTiming(
...
bool ImageSynchroniserNodelet::ExecuteCommandSoftwareTrigger() {
  std::vector<std::vector<sensor_msgs::ImageConstPtr>> temp(
      image_grouper_ptrs_.size(), std::vector<sensor_msgs::ImageConstPtr>());
  ...
  while (ros::ok()) {
    ...
    if (is_running) {
      ...
      if (!bad_data_) {
        ...
        bool success_this_iteration = (synchroniser_state_.is_hardware_trigger)
                                          ? ExecuteCommandHardwareTrigger()
        ...
        // If bad data, we clear all projector time / image buffers since we assume all incoming
        // data will be bad
        projector_time_buffer_.clear();
    ...
    ros::Duration(0.001f)
        .sleep();  // Need this sleep or else nodelet will not respond to service calls
  }
}
...
std::vector<std::string> ImageSynchroniserNodelet::SplitString(const std::string& s,
...
bool ImageSynchroniserNodelet::ProcessNotifyBadData(
    sl_sensor_image_acquisition::NotifyBadData::Request& req,
