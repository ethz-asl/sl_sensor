...
ImageGrouper::ImageGrouper(std::string image_sub_topic, int number_images_per_group,
                           double image_trigger_period, double lower_bound_tol,
                           double upper_bound_tol)
...
bool ImageGrouper::GetImagePtrFromBuffer(const ros::Time& target_time,
                                         sensor_msgs::ImageConstPtr& image_it) {
  bool result = false;
...
sensor_msgs::ImageConstPtr ImageGrouper::GetLatestImageAndClearBuffer() {
  ...
  auto pointer =
      sensor_msgs::ImageConstPtr((image_ptr_buffer_.empty()) ? nullptr : image_ptr_buffer_.back());
